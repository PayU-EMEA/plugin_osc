...
class payu_account
{
	...
	function payu_account()
	{
		...
		$this->signature = 'payu|payu_account|1.0.1|2.3.1';
		...
		$this->description = payu_version('1.0.1', '');
	...
	function install()
	{
		...
		tep_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('PayU Account', 'MODULE_PAYMENT_PAYU_ACCOUNT_VERSION', '1.0.1', '', '6', '0', '', 'payu_version(', now())");
	...
	function after_process()
	{
		...
		$redir = tep_href_link("ext/modules/payment/payu/auth.php", NULL, 'SSL').'%3Fpayu_session_id='.$payu_session['session_id'];
		tep_redirect(OpenPayU_Configuration::getAuthUrl().'?redirect_uri='.$redir.'&response_type=code&client_id='.urlencode(OpenPayU_Configuration::getClientId()));
	}
    ...
    {	
    	...
    	if(!empty($this->order->products))
    	{
	    	foreach($this->order->products as $product)
	    	{
				...
				$products[$i]['ShoppingCartItem']['Quantity'] = $product['qty'];
				...
				$this->total_price['Gross'] += $item_price['Gross'] * $product['qty'];
				$this->total_price['Net'] += $item_price['Net'];
				$this->total_price['Tax'] += $tax;
				$this->total_price['TaxRate'] = $tax_rate;
    ...
    private function create_order()
    {
    	...
    	(
	    	...
	    	(
				...
				'ShoppingCart' => array(
					'GrandTotal' => $this->order->info['total'] * 100,
					'CurrencyCode' => $this->order->info['currency'],
					'ShoppingCartItems' => $this->get_products_list()
			    ),
		...
		$this->response = OpenPayU_Order::create($order);

		#if response success is true return session_id 
		if($this->response->getSuccess() == TRUE)
		{
